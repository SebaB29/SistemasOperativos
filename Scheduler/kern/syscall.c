...
int
sys_get_tickets(struct Env *env)
{
	return env->env_tickets;
}

void
sys_set_tickets(struct Env *env)
{
	if (env->env_tickets == MIN_TICKETS) {
		env->env_tickets = MAX_TICKETS;
		env->env_cant_reseteos++;
	} else {
		env->env_tickets--;
	}
}
...
sys_exofork(void)
{
	...
	newenv->env_tickets = MAX_TICKETS;
